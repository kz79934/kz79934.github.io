...
if ($type == 'isotope') {
		...
		$container_classes[] = 'style-' . $style_preset;
		if ($inner_padding === 'yes') $parent_container_classes[] = 'justified-inner-padding';
		if ($gallery_back_color !== '') $parent_container_classes[] = 'style-'.$gallery_back_color.'-bg';
...
if (count($medias) > 0) {
	...
	foreach ($medias as $key_item => $item_thumb_id) {
		...
		if ( !in_array('expand', $detect_albums) )
			$item_prop = (isset($items[$old_medias[$key_item] . '_i'])) ? $items[$old_medias[$key_item] . '_i'] : '';
		...
		if ($type !== 'carousel') {
			...
			if (!$nested) $single_width = floor(12 / $carousel_lg);
		}
